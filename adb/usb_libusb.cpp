...
static int
usb_bulk_write(usb_handle *uh, uint8_t *data, int len)
{
    int r = 0;
    int transferred = 0;

    r = libusb_bulk_transfer(uh->devh, uh->end_point_address[1], data, len,
                             &transferred, 0);
...
static int
usb_bulk_read(usb_handle *uh, uint8_t *data, int len)
{
    int r = 0;
    int transferred = 0;
...
int
check_usb_interface(const libusb_interface *interface,
...
register_device(struct usb_handle *uh, const char *serial)
{
    ...
    usb = static_cast<struct usb_handle*>(calloc(1, sizeof(struct usb_handle)));
